...
namespace TeleRad.Shared.Utility;
...
public static class QueryHelper
{
        ...
        int? scopeUserId, int? userType)
    {
        ...
        {
            // Include dictation pool user (username='Dictation') if transcriber uses dictation pool
        ...
        sql += " ORDER BY s.dob DESC LIMIT 500";
        cmd.CommandText = sql;
        return cmd;
    }
    ...
    {
        var cmd = Cmd(c, @"
            SELECT s.*, s.pdf_report AS pdf_path,
                   r.displayname AS rad_name, t.displayname AS transcriber_name,
    ...
    public static MySqlCommand GetStudyPreview(MySqlConnection c, int studyId)
    {
        var cmd = Cmd(c, @"
            SELECT
                ...
                -- ref_u = the referring user (ref_id) — matches Laravel defaultTemplate / defaultTemplateWithImage
                ref_u.displayname AS client_name,
                ref_u.office      AS client_address,
                ref_u.phone       AS client_phone,
                ref_u.fax         AS client_fax,
                COALESCE(
                    -- 1. study's directly assigned template (Laravel: $this->template)
                    NULLIF((SELECT tmpl.header_image FROM tran_template tmpl WHERE tmpl.id = s.templateid LIMIT 1), ''),
                    -- 2. ref_id template with image (Laravel: defaultTemplateWithImage — userid=ref_id, header_image!='')
                    ...
                    -- 3. ref_id template any (Laravel: defaultTemplate — userid=ref_id)
    ...
    public static MySqlCommand UpdateClientInfo(MySqlConnection c, int id,
        ...
        int? countryId, int? stateId, int? cityId, int? defaultTranscriberId,
        string? username = null, int? userTypeId = null, string? dictationPool = null)
    {
        var sets = new List<string>();
        var cmd  = new MySqlCommand { Connection = c };
        ...
        if (countryId.HasValue)            Add("country",             "cid", countryId);
        if (stateId.HasValue)              Add("state",               "sid", stateId);
        if (cityId.HasValue)               Add("city",                "cty", cityId);
        if (defaultTranscriberId.HasValue) Add("default_transcriber", "dt",  defaultTranscriberId);
        if (username != null)              Add("username",            "un",  username);
        if (userTypeId.HasValue)           Add("usertype",            "ut",  userTypeId);
        if (dictationPool != null)         Add("auto_sign",           "dp",  dictationPool);
        if (sets.Count == 0) { cmd.CommandText = "SELECT 1"; return cmd; }
        cmd.CommandText = $"UPDATE tran_user SET {string.Join(", ", sets)} WHERE id = @id";
        cmd.Parameters.AddWithValue("@id", id);
        return cmd;
    ...
    public static MySqlCommand UpdateUserPassword(MySqlConnection c, int id, string passwordHash, string? passwordString = null)
    {
        // When called from user self-change: passwordString is null → is_password_reset = 0
        // When called from admin reset:      passwordString provided → store plain + is_password_reset = 1
        string sql = passwordString != null
            ? "UPDATE tran_user SET password = @pwd, password_string = @ps, is_password_reset = '1' WHERE id = @id"
            : "UPDATE tran_user SET password = @pwd, is_password_reset = '0' WHERE id = @id";
        var cmd = Cmd(c, sql);
        cmd.Parameters.AddWithValue("@pwd", passwordHash);
        if (passwordString != null) cmd.Parameters.AddWithValue("@ps", passwordString);
        cmd.Parameters.AddWithValue("@id", id);
        return cmd;
    }
    ...
    {
        var cmd = new MySqlCommand { Connection = c };
        // Match Laravel Spatie Role — automatically filters by guard_name = 'web'
        var sql = "SELECT id, name FROM tran_user_type WHERE guard_name = 'web'";
        if (!string.IsNullOrEmpty(search)) { sql += " AND name LIKE @s"; cmd.Parameters.AddWithValue("@s", $"{search}%"); }
    ...
    {
        var cmd = new MySqlCommand { Connection = c };
        // No del filter — tran_attachment has no del column (matches Laravel document() relationship)
        var sql = @"
            ...
            FROM tran_attachment WHERE exam_id = @sid";
    ...
    {
        var cmd = new MySqlCommand { Connection = c };
        // Laravel filters on type_rad = 'send audio to transcription' + rad_id, JOINs tran_attachment
        var sql = @"
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // FAX
    // fax_sent:    fax_sid, status, created_at, delevered_at
    // tran_faxed_doc: id, sid (→ fax_sent.fax_sid), fax (number), attachment (file)
    // fax_receiveds:  id, from, media_file, created_at, modified_at, ref_id
    // ══════════════════════════════════════════════════════════════════════════

    public static MySqlCommand InsertFaxSent(MySqlConnection c, string faxNumber, string fileName, int? studyId)
    {
        // Insert into tran_faxed_doc + fax_sent pair (fax_sid links them)
        var cmd = Cmd(c, @"
    ...
    {
        // Laravel: tran_faxed_doc JOIN fax_sent on sid = fax_sid
        var cmd = Cmd(c, @"
    ...
    /// <summary>
    /// Bulk lookup of fax_receiveds records by a list of S3 media_file paths.
    /// Used by GetInboundFaxesAsync to mirror Laravel's per-file DB lookup.
    /// </summary>
    ...
    {
        // Rename media_file entry in fax_receiveds
    ...
    public static MySqlCommand GetChartPendingByTranscriber(MySqlConnection c, int? radId)
    {
        var cmd = new MySqlCommand { Connection = c };
        var sql = @"
    ...
    public static MySqlCommand GetChartOnHoldByTranscriber(MySqlConnection c, int? radId)
    {
        var cmd = new MySqlCommand { Connection = c };
        var sql = @"
            SELECT u.displayname AS label, COUNT(*) AS total
    ...
    public static MySqlCommand GetChartOnHoldByRadiologist(MySqlConnection c, int? radId)
    {
        var cmd = new MySqlCommand { Connection = c };
        var sql = @"
    ...
    public static MySqlCommand GetChartPendingSignatureByRadiologist(MySqlConnection c, int? radId)
    {
        var cmd = new MySqlCommand { Connection = c };
        var sql = @"
            SELECT COALESCE(u.displayname, '') AS label, COUNT(*) AS total
    ...
    public static MySqlCommand GetChartNoAudioFile(MySqlConnection c, int? radId)
    {
        var cmd = new MySqlCommand { Connection = c };
    ...
    {
        var cmd = new MySqlCommand { Connection = c };
        // Normalise status: accept both underscore form ("hold_for_comparison") and
        // space form ("hold for comparison") so old and new DB rows both match.
        var sql = @"
            SELECT s.id,
            ...
            WHERE (s.type = @status OR s.type = REPLACE(@status,'_',' '))
              AND s.`delete` = 0";

        // Scope filter per user type
        // userType 2 = radiologist → filter by rad_id
        // userType 3 = transcriber → filter by trans_id
        // userType 4 = referrer    → filter by ref_id
        // userType 1 = admin       → no filter, sees all records (matches live site behaviour)
        if (userType == 2 && scopeUserId.HasValue)
        {
            sql += " AND s.rad_id = @sid";
            cmd.Parameters.AddWithValue("@sid", scopeUserId.Value);
        }
        else if (userType == 3 && scopeUserId.HasValue)
        {
            sql += " AND s.trans_id = @sid";
            cmd.Parameters.AddWithValue("@sid", scopeUserId.Value);
        }
        else if (userType == 4 && scopeUserId.HasValue)
        {
            sql += " AND s.ref_id = @sid";
            cmd.Parameters.AddWithValue("@sid", scopeUserId.Value);
        }

        sql += " ORDER BY s.dob DESC LIMIT @limit OFFSET @offset";
        cmd.Parameters.AddWithValue("@status", status);
        cmd.Parameters.AddWithValue("@limit",  perPage);
        cmd.Parameters.AddWithValue("@offset", offset);
        cmd.CommandText = sql;
    ...
    // Batch: all listed statuses in one SQL query (used by combined-view)
    ...
    {
        var cmd = new MySqlCommand { Connection = c };
        // Build IN clause: each status matches both underscore and space form
    ...
    public static MySqlCommand GetStudiesBatchCount(MySqlConnection c, IList<string> statuses, int? scopeUserId, int? userType)
    {
        var cmd = new MySqlCommand { Connection = c };
        var conditions = new System.Text.StringBuilder();
        for (int i = 0; i < statuses.Count; i++)
        ...
        var sql = $"SELECT COUNT(*) FROM tran_typewordlist s WHERE ({conditions}) AND s.`delete` = 0";
        if (userType == 2 && scopeUserId.HasValue) { sql += " AND s.rad_id = @sid";   cmd.Parameters.AddWithValue("@sid", scopeUserId.Value); }
        else if (userType == 3 && scopeUserId.HasValue) { sql += " AND s.trans_id = @sid"; cmd.Parameters.AddWithValue("@sid", scopeUserId.Value); }
        else if (userType == 4 && scopeUserId.HasValue) { sql += " AND s.ref_id = @sid";   cmd.Parameters.AddWithValue("@sid", scopeUserId.Value); }
        cmd.CommandText = sql;
    ...
    public static MySqlCommand GetStudiesByStatusCount(MySqlConnection c, string status, int? scopeUserId, int? userType)
    {
        var cmd = new MySqlCommand { Connection = c };
        var sql = @"SELECT COUNT(*) FROM tran_typewordlist s
                    WHERE (s.type = @status OR s.type = REPLACE(@status,'_',' '))
                      AND s.`delete` = 0";
        if (userType == 2 && scopeUserId.HasValue) { sql += " AND s.rad_id = @sid";  cmd.Parameters.AddWithValue("@sid", scopeUserId.Value); }
        else if (userType == 3 && scopeUserId.HasValue) { sql += " AND s.trans_id = @sid"; cmd.Parameters.AddWithValue("@sid", scopeUserId.Value); }
    ...
    {
        // Same as Laravel getDistinctModalitiesNew() — no status filter, all studies
        return Cmd(c, @"
    ...
    /// <summary>
    /// Returns exams from tran_exam filtered by modality.
    /// tran_typewordlist.examid references tran_exam.id.
    /// </summary>
    public static MySqlCommand GetExamsByModality(MySqlConnection c, string? modality, string? search)
    {
        var cmd = new MySqlCommand { Connection = c };
        var sql = @"
            SELECT e.id, e.exam
            FROM tran_exam e
            INNER JOIN tran_modality m ON m.id = e.modalityid
            WHERE e.exam IS NOT NULL
              AND e.exam <> ''";
        if (!string.IsNullOrWhiteSpace(modality))
        {
            sql += " AND m.modality = @mod";
            cmd.Parameters.AddWithValue("@mod", modality.Trim());
        }
        if (!string.IsNullOrWhiteSpace(search))
        {
            sql += " AND e.exam LIKE @s";
            cmd.Parameters.AddWithValue("@s", $"%{search.Trim()}%");
        }
        sql += " ORDER BY e.exam ASC LIMIT 100";
        cmd.CommandText = sql;
        return cmd;
    }
    ...
    {
        var cmd = new MySqlCommand { Connection = c };
        // Laravel: radiologist filters by type='rad non dicom accunts', admin by type_rad='non dicom accounts from admin'
        var sql = @"
            SELECT tw.id, tw.pname AS patient_name, tw.firstname, tw.lastname,
                   tw.idnumber, tw.dos, tw.dob, tw.type AS status, tw.modality,
                   tw.description AS exam, tw.ordering_physician, tw.location,
                   tw.access_number, tw.rad_id, tw.ref_id AS client_id,
                   u.displayname AS client_name, t.displayname AS transcriber_name
            FROM tran_typewordlist tw
    ...
    public static MySqlCommand InsertNonDicomEntry(MySqlConnection c,
        string patientName, string? firstName, string? lastName,
        string? dob, string? dos, string? modality, string? examType,
        string? description, int? clientId, int? radId,
        string? orderingPhysician, string? location, string? accessNumber, string? idNumber)
    {
        // Matches Laravel Study::insertNonDicomEntry():
        //   type     = 'rad non dicom accunts'  (intentional typo in original)
        //   type_rad = 'non dicom accounts from admin'
        //   after insert: incoming_order_patient_id = incoming_order_exam_id = own id
        var cmd = Cmd(c, @"
            INSERT INTO tran_typewordlist
                (pname, firstname, lastname, dob, dos, modality, type, type_rad,
                 description, ref_id, rad_id, idnumber, access_number,
                 ordering_physician, location, trans_new_message_time, `delete`)
            VALUES
                (@pn, @fn, @ln, @dob, @dos, @mod,
                 'rad non dicom accunts', 'non dicom accounts from admin',
                 @desc, @cid, @rid, @idn, @acc, @op, @loc, NOW(), 0)");
        ...
        cmd.Parameters.AddWithValue("@cid", (object?)clientId    ?? DBNull.Value);
        cmd.Parameters.AddWithValue("@rid", (object?)radId       ?? DBNull.Value);
        cmd.Parameters.AddWithValue("@idn", (object?)idNumber    ?? DBNull.Value);
        cmd.Parameters.AddWithValue("@acc", (object?)accessNumber ?? DBNull.Value);
        cmd.Parameters.AddWithValue("@op",  (object?)orderingPhysician ?? DBNull.Value);
        cmd.Parameters.AddWithValue("@loc", (object?)location    ?? DBNull.Value);
        return cmd;
    }

    /// <summary>
    /// After INSERT, set incoming_order_patient_id = incoming_order_exam_id = own id.
    /// Matches Laravel's post-save logic in insertNonDicomEntry().
    /// </summary>
    public static MySqlCommand SetNonDicomIncomingIds(MySqlConnection c, long id)
    {
        var cmd = Cmd(c, @"
            UPDATE tran_typewordlist
            SET incoming_order_patient_id = @id,
                incoming_order_exam_id    = @id
            WHERE id = @id");
        cmd.Parameters.AddWithValue("@id", id);
        return cmd;
    }
    ...
    /// <summary>
    /// Mirrors Laravel updateNonDicomEntry() when trans_id is provided:
    ///   trans_id set, type = 'trans new messages', trans_new_message_time = NOW()
    /// </summary>
    public static MySqlCommand InterpretNonDicomEntry(MySqlConnection c, int studyId, int transId)
    {
        var cmd = Cmd(c, @"
            UPDATE tran_typewordlist
            SET trans_id                = @tid,
                type                    = 'trans new messages',
                trans_new_message_time  = NOW()
    ...
    /// <summary>
    /// Insert audio attachment into tran_attachment.
    /// Mirrors Laravel Attachment::insert() in updateNonDicomEntry().
    /// </summary>
    public static MySqlCommand InsertAudioAttachment(MySqlConnection c, int studyId, string filePath)
    {
        var cmd = Cmd(c, @"
            INSERT INTO tran_attachment
                (exam_id, patient_id, curr_date, attachment, attachment_type, status)
            VALUES
                (@eid, @pid, NOW(), @path, 'Audio File', 'trans new messages')");
        cmd.Parameters.AddWithValue("@eid",  studyId);
        cmd.Parameters.AddWithValue("@pid",  studyId);
