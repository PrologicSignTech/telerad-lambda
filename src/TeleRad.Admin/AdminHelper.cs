...
public class AdminHelper : HelperBase
{
    ...
    public async Task<APIGatewayProxyResponse> GetInboundFaxesAsync(string? body)
    {
        ...
        try
        {
            // --- Step 1: list S3 root-level files in php/faxesin/ ---
            ...
            do
            {
                var listReq = new ListObjectsV2Request
                {
                    BucketName = S3Bucket,
                    Prefix     = "php/faxesin/",
                    Delimiter  = "/",           // non-recursive: root only
            ...
            if (s3Files.Count == 0)
                return FunctionBase.Ok(new List<InboundFaxResponse>());
            ...
            while (await reader.ReadAsync())
            {
                ...
                dbMap[path] = (
                    ...
                    SafeDateStr(reader, "modified_at")
                );
            }
            ...
            foreach (var s3f in s3Files.OrderByDescending(f => f.LastModified))
            {
                ...
                {
                    // File in S3 but no DB record — mirror Laravel's else branch
